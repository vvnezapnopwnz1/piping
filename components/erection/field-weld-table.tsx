...
interface FieldWeldTableProps {
  data: FieldWeldJoint[];
  onSelectJoint: (joint: FieldWeldJoint) => void;
  selectedJointId?: string;
}

type SortKey = keyof FieldWeldJoint;
...
export function FieldWeldTable({
  ...
  const filtered = data
    .filter((row) => {
      if (
        !scope.isInScope(
          (row as FieldWeldJoint & { pdsAreaCode?: string }).pdsAreaCode,
        )
      ) {
        return false;
      }
      ...
      return (
        ...
        row.erectionStatus.toLowerCase().includes(q) ||
        row.areaZone.toLowerCase().includes(q) ||
        row.fieldJointType.toLowerCase().includes(q)
      );
    })
    .sort((a, b) => {
      if (!sortKey || !sortDir) return 0;
      const av = String(a[sortKey as keyof FieldWeldJoint] ?? "");
      const bv = String(b[sortKey as keyof FieldWeldJoint] ?? "");
  ...
  const SortIcon = ({ col }: { col: SortKey }) => {
    ...
    if (sortDir === "asc")
      return <ChevronUp className="w-3 h-3 text-sky-600 ml-1" />;
  ...
  return (
    <div className="flex flex-col h-full min-w-0 w-full overflow-hidden rounded-xl border border-slate-200 bg-white">
        ...
        <table className="w-full border-collapse text-sm min-w-[760px]">
          <thead className="sticky top-0 z-10">
            <tr className="bg-slate-100 border-b border-slate-200">
              ...
              <ColHeader label="Area Zone" col="areaZone" />
              ...
              <ColHeader label="Joint Type" col="fieldJointType" />
            ...
            {filtered.map((row, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-2 py-2 whitespace-nowrap">
                    <span className="text-xs text-slate-700 font-mono inline-flex items-center">
                      {row.welderCode}
                      {(() => {
      ...
      <div className="px-3 py-1.5 border-t border-slate-200 flex items-center justify-between flex-shrink-0">
        ...
        <span className="text-xs text-slate-600">
          Click a row or <Pencil className="w-3 h-3 inline" /> to open detail
          panel
        </span>
      </div>
    </div>
