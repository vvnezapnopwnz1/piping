-- PostgreSQL does not define min(uuid). Keep the command contract unchanged and
-- resolve the sole accepted revision through an ordered array instead.
...
begin
  ...
  select count(*)::integer, (array_agg(revision.id order by revision.id))[1]
  ...
  insert into public.spool_location_events (
    project_id, spool_id, spool_revision_id, location_id, device_id,
    operator_membership_id, direction, occurred_at, source,
    source_import_job_id, source_event_key, compensates_event_id, reason, recorded_by
  ) values (
    p_project_id, p_spool_id, accepted_revision_id, p_location_id, p_device_id,
    p_operator_membership_id, p_direction, p_occurred_at, p_source,
    p_source_import_job_id, trim(p_source_event_key), p_compensates_event_id,
    nullif(trim(coalesce(p_reason, '')), ''), p_recorded_by
  )
